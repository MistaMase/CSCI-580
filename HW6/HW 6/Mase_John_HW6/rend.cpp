...
GzRender::GzRender(int xRes, int yRes)
{
	...
	for (int aa_num = 0; aa_num < AAKERNEL_SIZE; aa_num++) {
		aa_buffer[aa_num].pixel_offset = new GzPixel[xRes * yRes];
	}
...
GzRender::~GzRender()
{
	...
	// Deallocate the anti-aliasing buffer
	for (int aa_num = 0; aa_num < AAKERNEL_SIZE; aa_num++) {
		delete[] aa_buffer[aa_num].pixel_offset;
	}
}
...
int GzRender::GzDefault()
{
	...
	// Set default aa buffer values to slightly blue with no alpha
	for (int idx = 0; idx < xres * yres; idx++) {
		for (int aa_num = 0; aa_num < AAKERNEL_SIZE; aa_num++) {
			aa_buffer[aa_num].pixel_offset[idx].red = (GzIntensity)160 << 4;
			aa_buffer[aa_num].pixel_offset[idx].green = (GzIntensity)218 << 4;
			aa_buffer[aa_num].pixel_offset[idx].blue = (GzIntensity)235 << 4;
			aa_buffer[aa_num].pixel_offset[idx].alpha = 4095;
			aa_buffer[aa_num].pixel_offset[idx].z = INT_MAX;
		}
...
/* Pixelbuffer/Anti-Aliasing Buffer Access */

int GzRender::GzPut(int i, int j, GzIntensity r, GzIntensity g, GzIntensity b, GzIntensity a)
{
...
int GzRender::GzPutAA(int i, int j, GzIntensity r, GzIntensity g, GzIntensity b, GzIntensity a, GzDepth z, int kernel_offset) {
	// Ensure pixel index is within screen-space bounds
	if (i >= 0 && i < xres && j >= 0 && j < yres) {
		int idx = xres * j + i;

		// Bound red to 0-4095
		if (r > 4095) aa_buffer[kernel_offset].pixel_offset[idx].red = 4095;
		else if (r < 0)	aa_buffer[kernel_offset].pixel_offset[idx].red = 0;
		else aa_buffer[kernel_offset].pixel_offset[idx].red = r;

		// Bound green to 0-4095
		if (g > 4095) aa_buffer[kernel_offset].pixel_offset[idx].green = 4095;
		else if (g < 0) aa_buffer[kernel_offset].pixel_offset[idx].green = 0;
		else aa_buffer[kernel_offset].pixel_offset[idx].green = g;

		// Bound blue to 0-4095
		if (b > 4095) aa_buffer[kernel_offset].pixel_offset[idx].blue = 4095;
		else if (b < 0)	aa_buffer[kernel_offset].pixel_offset[idx].blue = 0;
		else aa_buffer[kernel_offset].pixel_offset[idx].blue = b;

		// Bound alpha to 0-4095
		if (a > 4095) aa_buffer[kernel_offset].pixel_offset[idx].alpha = 4095;
		else if (a < 0)	aa_buffer[kernel_offset].pixel_offset[idx].alpha = 0;
		else aa_buffer[kernel_offset].pixel_offset[idx].alpha = a;

		// Bound Z from 0 - INT_MAX
		if (z > INT_MAX) aa_buffer[kernel_offset].pixel_offset[idx].z = INT_MAX;
		else if (z < 0)	aa_buffer[kernel_offset].pixel_offset[idx].z = 0;
		else aa_buffer[kernel_offset].pixel_offset[idx].z = z;
	}
...
int GzRender::GzPutTriangle(int numParts, GzToken *nameList, GzPointer *valueList)
{
	...
	*/
	...
	for (int token_num = 0; token_num < numParts; token_num++) {
		...
		// Triangle Normal - Calls the renderer if there's no given texture function
		...
		// Triangle UV Index - Calls the renderer as now we have a vertex, normal, and a uv index
			...
			// Render here since we have all the necessary information
			GzRenderTriangle();
		}
	}
...
int GzRender::GzRenderTriangle() {
	...
	// Only skip this triangle if all anti-aliased coordinates of the triangle are outside of the view range
	for (int aa_num = 0; aa_num < AAKERNEL_SIZE; aa_num++) {
		// Apply the current transformation to each vertex for error checking
		GzCoord transformed_vertices[3];
		matrix_vector_multiply_offset(last_triangle.vertices[0], Ximage[matlevel - 1], transformed_vertices[0], AAFilter[aa_num][X], AAFilter[aa_num][Y]);
		matrix_vector_multiply_offset(last_triangle.vertices[1], Ximage[matlevel - 1], transformed_vertices[1], AAFilter[aa_num][X], AAFilter[aa_num][Y]);
		matrix_vector_multiply_offset(last_triangle.vertices[2], Ximage[matlevel - 1], transformed_vertices[2], AAFilter[aa_num][X], AAFilter[aa_num][Y]);
	...
	// Combine the samples from anti-aliasing
	for (int pixel = 0; pixel < xres * yres; pixel++) {
		GzIntensity r = 0, g = 0, b = 0, a = 0;
		for (int aa_num = 0; aa_num < AAKERNEL_SIZE; aa_num++) {
			r += (GzIntensity)(aa_buffer[aa_num].pixel_offset[pixel].red * AAFilter[aa_num][2]);
			g += (GzIntensity)(aa_buffer[aa_num].pixel_offset[pixel].green * AAFilter[aa_num][2]);
			b += (GzIntensity)(aa_buffer[aa_num].pixel_offset[pixel].blue * AAFilter[aa_num][2]);
			a += (GzIntensity)(aa_buffer[aa_num].pixel_offset[pixel].alpha * AAFilter[aa_num][2]);
		}
		GzPut(pixel % xres, pixel / xres, r, g, b, a);
	}
...
int GzRender::GzRasterizeTriangle(GzCoord transformed_vertices[3], int aa_num) {
	// Save status
	int status = GZ_SUCCESS;
	...
	status |= GzScanLine(cur, span, on_left, long_edge_slope_xy, long_edge_slope_zy, intercept_xy, intercept_zy, aa_num);
...
int GzRender::GzScanLine(DDA& cur, DDA_Span& span, bool on_left, float long_edge_slope_xy, float long_edge_slope_zy, float intercept_xy, float intercept_zy, int aa_num) {
	// Save status
	int status = GZ_SUCCESS;
	...
	while (cur.current[Y] < cur.end[Y]) {
		...
		while (span.current[0] < span.end[0]) {
			...
			if (span.current[0] >= 0 && span.current[0] < xres && cur.current[Y] >= 0 && cur.current[Y] < yres && (GzDepth)span.current[1] < aa_buffer[aa_num].pixel_offset[ARRAY((int)span.current[0], (int)cur.current[Y])].z) {
				...
				// Place the pixel color into the AA Buffer
				status |= GzPutAA((int)span.current[0], (int)cur.current[Y], ctoi(color[RED]), ctoi(color[GREEN]), ctoi(color[BLUE]), 4095, (GzDepth)span.current[1], aa_num);
			}
